...
namespace Infrastructure.Managers
{
    ...
    public class InputManager : GameService, IInputManager
    {
        ...
        public bool MouseHover(Rectangle i_SourceRectanle)
        {
            bool hover = false;

            if ((MouseState.X >= i_SourceRectanle.Left && MouseState.X <= i_SourceRectanle.Right)
                && (MouseState.Y >= i_SourceRectanle.Top && MouseState.Y <= i_SourceRectanle.Bottom))
            {
                hover = true;
            }
